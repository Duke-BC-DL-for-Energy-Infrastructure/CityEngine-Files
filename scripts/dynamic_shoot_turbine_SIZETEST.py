...
def get_turbine_sizes(file_path):
...
#    height = setCamHeight(tile_width=TileSize)
    ...
    print('camera elevation: {} deg'.format(camera_angles[0][1:]))
...
#            # for sd in tqdm(range(ite_num), leave=True):
#            for sd in range(ite_num):
#                turbine_size_params = {'LenMinModelFW' : size, 'LenMaxModelFW' : size}
#                rule = rgb_rule_file[0]
#                load_rule_file(sd, rule, turbine_size_params)
#                parent_folder = 'turbine_sizetest_{}'.format(size)
#                print(parent_folder)
#                
#                try: 
